...
export default function ResultCard({ plan, onReset }: ResultCardProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl bg-subway-card p-6 shadow-xl max-w-md mx-auto"
    >
      {/* Weather Summary */}
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-3">
          <Badge color="a" size="lg">{weather.city.charAt(0).toUpperCase()}</Badge>
          <h2 className="text-h2 font-semibold text-subway-text">{weather.city}</h2>
        </div>
        
        <div className="bg-subway-surface rounded-lg p-4 mb-4">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-subway-text">{weather.avg}°</div>
              <div className="text-xs text-subway-muted">Average</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-subway-text">{weather.min}°</div>
              <div className="text-xs text-subway-muted">Min</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-subway-text">{weather.max}°</div>
              <div className="text-xs text-subway-muted">Max</div>
            </div>
          </div>
          <p className="text-sm text-subway-muted mt-3 text-center">{weather.summary}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {packing.outerwear.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-subway-text font-medium">Outerwear</span>
            </div>
            <div className="ml-10 space-y-1">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-subway-text font-medium">Footwear</span>
          </div>
          <div className="ml-10 space-y-1">
        ...
        {packing.accessories.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-subway-text font-medium">Accessories</span>
            </div>
            <div className="ml-10 space-y-1">
      ...
      {/* Notes */}
      {notes.length > 0 && (
        <div className="mt-6 p-4 bg-subway-surface rounded-lg">
          <h4 className="text-sm font-semibold text-subway-text mb-2">Notes</h4>
          <ul className="space-y-1">
            {notes.map((note, index) => (
              <li key={index} className="text-sm text-subway-muted">• {note}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <button
        onClick={onReset}
        className="w-full mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-subway-surface border border-subway-muted px-5 py-3 font-semibold text-subway-text hover:border-subway-a transition-colors"
      >
        Plan Another Trip
